...
export default function Header({ 
  ...
  useEffect(() => {
    ...
    if (userStr) {
      ...
      // Kullanıcı verisi yüklendiğinde bildirimlerini çek
      fetchNotifications(parsedUser.id);
    }
  }, []);

  // Backend'den bildirimleri çekme fonksiyonu
  ...
  const markAsRead = async (notifId: number) => {
    try {
      ...
      // Okundu işaretledikten sonra listeyi arayüzde anında güncelle
    ...
    <header className="flex flex-col bg-background shrink-0 transition-colors duration-300 relative z-30">
      <div className="h-16 flex items-center justify-between px-6 border-b border-border-main">
        ...
        <div className="flex items-center gap-4 ml-4 shrink-0 relative">
          ...
          {/* YENİ: BİLDİRİM ÇANI VE DROPDOWN */}
          <div className="relative">
            <button 
              onClick={() => setIsNotifOpen(!isNotifOpen)} 
              className={`p-1 transition-colors rounded-full ${isNotifOpen ? 'bg-surface text-text-main' : 'text-text-muted hover:text-text-main'}`}
            >
              <Bell className="w-5 h-5" />
              {/* Sadece okunmamış bildirim varsa kırmızı noktayı göster */}
              {unreadCount > 0 && (
                <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-pink-500 border-2 border-background rounded-full animate-pulse"></span>
              )}
            </button>
            ...
            {isNotifOpen && (
              <>
                {/* Dışarı tıklamayı algılamak için görünmez arka plan */}
                          ...
                          >
                            <div className="mt-0.5">
                              {notif.is_read ? (
                                ...
                                <div className="w-2 h-2 mt-1.5 bg-primary rounded-full shadow-[0_0_8px_rgba(59,130,246,0.6)]"></div>
                              )}
                            </div>
                            <div className="flex-1">
                              <p className={`text-sm ${notif.is_read ? 'text-text-muted' : 'text-text-main font-medium'}`}>
                                {notif.message}
                              </p>
      ...
      <div className="h-16 flex items-center justify-between px-6 border-b border-border-main">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-semibold text-text-main tracking-tight">
            {isBoardPage ? 'Team Workspace' : 'User Profile'}
          </h1>
          {isBoardPage && (
            <span className="px-2.5 py-0.5 rounded-full bg-primary/20 text-primary text-xs font-medium border border-primary/10">Active</span>
          )}
        </div>
