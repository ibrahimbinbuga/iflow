...
export default function Sidebar() {
  // Company menüsünün açık/kapalı durumunu tutan state
  const [isCompanyOpen, setIsCompanyOpen] = useState(true);

  return (
    <aside className="w-64 bg-[#121212] border-r border-[#27272A] h-screen flex flex-col text-sm shrink-0">
      <div className="h-16 flex items-center px-6 font-bold text-lg tracking-wide text-white gap-2 border-b border-[#27272A]/50">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-6 custom-scrollbar">
        <div>
          ...
          <div className="space-y-1">
            ...
            {/* Company Menüsü ve Toggle İşlevi */}
            <div className="pt-1">
              <button 
                onClick={() => setIsCompanyOpen(!isCompanyOpen)}
                className="w-full flex items-center justify-between px-3 py-2 text-white bg-surface rounded-md transition-colors font-medium"
              >
              ...
              {/* Alt Projeler (Sadece isCompanyOpen true ise görünür) */}
              {isCompanyOpen && (
                <div className="ml-5 mt-1 border-l border-[#27272A] pl-2 space-y-0.5 overflow-hidden animate-in slide-in-from-top-2 duration-200">
